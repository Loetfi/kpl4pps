		...
		try { 
			if (is_array($value_update)) {
				...
				if ($value_update['is_main_address']==1) {
					...
					AddressDB::where('id_user_profile',$users->id_user_profile)->update(['is_main_address'	=> 0]);
					#kemudian baru menjadikan alamat yang diubah menjadi main address menjadi main address sesungguhnya.
					// dd(22);
					return AddressDB::where('id_user_address', $id_user_address)->where('id_user_profile',$users->id_user_profile)->update($value_update);
				} else {
					// return '';
					return AddressDB::where('id_user_address', $id_user_address)->where('id_user_profile',$users->id_user_profile)->update([
