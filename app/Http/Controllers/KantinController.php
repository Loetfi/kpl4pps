...
use App\Models\Anggota\KantinModel AS KantinModel;
use App\Helpers\Api;
...
class KantinController extends Controller
{
	...
	public function data(Request $request){
		try { 
			...
			$data_res = KantinModel::skip($request->offset)->take($request->limit)->orderby('nama','asc')->get();
			...
			foreach ($data_res as $harga) {
				...
				$data['kode_tenant'] = $harga->kode_tenant;
	...
	public function detail(Request $request){
		try { 
			...
			$data_res = KantinModel::where('id',$id)->first();
	...
	public function related(Request $request){
		try { 
			...
			$data_res = KantinModel::where('namakategori',$namakategori)->where('id','!=',$id)->take(5)->get();
	...
	public function searching(Request $request){
		try { 
			...
			$data_res = KantinModel::where('nama','like','%'.$nama.'%')->orwhere('namakategori','like','%'.$nama.'%')->skip($request->offset)->take($request->limit)->get();
	...
	public function list_kategori(Request $request){
		try { 
			// $data_res = KantinModel::select('namakategori as id' , 'namakategori as nama')->groupby('namakategori')->get();
			$data_res = KantinModel::select('namakategori as id' , 'namakategori as nama')->whereNotIn('namakategori',['107-01','KONSINYASI','piutang 2019'])->groupby('namakategori')->get();
	...
	public function pilih_kategori(Request $request){
		try { 
			...
			$data_res = KantinModel::where('namakategori','like','%'.$nama.'%')->skip($request->offset)->take($request->limit)->get();
	...
	public function buy(Request $request){
		try { 
			...
			if($istoko){
					...
					$chatId = "-384536993";
					$txt   ="#kantin #IDORDER-".$id_order." <strong>Order Baru dari Kantin </strong>"."\n";
					$txt .="----------------------------\n";
					$number = 1;
					foreach ($cart as $c) {
						
						$txt .="| Keranjang (".$number.") | "."\n";
						$txt .="| Nama Tenant : ".$c['kode_tenant']." | "."\n";
