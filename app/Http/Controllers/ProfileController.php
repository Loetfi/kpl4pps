			...
			public function edit(Request $request){
				try { 
					...
					$this->validate($request, [
						...
						$anggotaData = AnggotaModel::where('id',$anggota_id);
						if (!empty($request->has('password'))) {
							...
							if ($request->password === $request->password_confirm) {
								...
								$anggotaData->update($change_password);
							} else {
						...
						// jika id anggota username yang sama di post, maka di allow saja 
						$check_username =  ($check->get()->first());
						$nohpLama = $anggotaData->get()->first()->nohp;
						// dd($check->get()->first()->no_hp);
						...
						if ($update_username || $update_profile) {
							...
							$anggotaData = $anggotaData->get()->first();
							/**
							 * success edit
							 */
							...
							$chatId = "-498099621";
							$txt   ="#ganti_profil <strong>Ganti Profil </strong>"."\n";
							$txt  .="| No HP : ". $no_hp ."\n";
							$txt  .="| No HP Lama : ". $nohpLama ."\n";
							$txt  .="| Alamat : ". $alamat ."\n";
							$txt .="| Dari Nama Anggota : ".$anggotaData->nama."\n";
							$txt .="| Dari No Anggota : ".$anggotaData->noanggota."\n"; 
